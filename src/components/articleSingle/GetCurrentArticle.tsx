...
const GetSingleArticle = ({ idOrSlug }: GetSingleArticleProps) => {
  ...
  store.subscribe(() => {
    ...
    setCurrentTime(
      format(
        new Date(parseInt(store.getState().currentArticle?.timeStamp || '0')),
        'dddd MMMM Do, YYYY'
      )
    );
  });
        ...
        <div
          ...
          onClick={() => {
            for (
              ...
              (itemSearch as HTMLElement).style.display = 'none';
            }
          }}
        >
              ...
              <Link href={'/' + article.slug}>
                ...
                {article.timeStamp && (
                  <h5 className="text-right">
                    {format(new Date(parseInt(article.timeStamp || '0')), 'dddd MMMM Do, YYYY')}
                  </h5>
                )}
  ...
  const [userDetails, setUserDetails] = useState<{ email?: string } | undefined>(undefined);
        ...
        {article.error === 404 ? (
          ...
          <>
            ...
            {userConnected && userDetails?.email === 'kenneth7e7a@gmail.com' && (
              <div className="w-full flex gap-4 mt-4 p-2">
                <Link
                  href={'/crear?id=' + article.id}
